...
router.post("/tbdorder/:orderId", verifyToken, async (req, res) => {
  try {
    ...
    const orderdetails = await db
      .collection("Order")
      .findOne({ _id: mongodb.ObjectId(req.params.orderId) });
...
router.put(
  "/labpartner/orderReceived/:orderId",
  verifyToken,
  async (req, res) => {
    try {
      const db = await mongoConnect();
      ...
      const order = await db
        .collection("Order")
        .updateOne(
          { _id: mongodb.ObjectId(orderdetails.TBDorderid) },
          { $set: req.body }
        );
...
//Order shipped by Lab
router.put(
  "/labpartner/orderShipped/:orderId",
  verifyToken,
  async (req, res) => {
    try {
      const db = await mongoConnect();
...
router.put(
  "/labpartner/customerReceived/:orderId",
  verifyToken,
  async (req, res) => {
    try {
      const db = await mongoConnect();
...
router.put(
  "/labpartner/sampleReceived/:orderId",
  verifyToken,
  async (req, res) => {
    try {
      const db = await mongoConnect();
...
//result ready
router.put(
  "/labpartner/resultReady/:orderId",
  verifyToken,
  async (req, res) => {
    try {
      const db = await mongoConnect();
